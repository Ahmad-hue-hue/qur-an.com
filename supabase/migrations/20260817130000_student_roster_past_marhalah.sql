-- Bug: get_student_marhalah_results_roster required current_marhalah to
-- exactly equal the requested marhalah number, raising 'Student only' for
-- anything else. Once a student is promoted, they could no longer view
-- their own PAST marhalah's results at all - switching the results-page
-- filter to a previous marhalah simply failed. Fixed to allow any marhalah
-- the student has actually reached (current or already passed), matching
-- the same "<= assigned stage" rule is_marhalah_unlocked already uses for
-- content access.
