...
export default function AdminExerciseDetailPage({
  ...
  const { data: exercise, isLoading } = useQuery({
    queryKey: ["teacher-exercise", exerciseId],
    queryFn: () => teacherApi.getExercise(exerciseId),
  });

  const { data: submissions } = useQuery({
    queryKey: ["teacher-exercise-submissions", exerciseId],
    queryFn: () => teacherApi.getExerciseSubmissions(exerciseId),
  ...
  const saveQuestionMutation = useMutation({
    mutationFn: () => {
      ...
      if (editingQuestionId) {
        return teacherApi.updateExerciseQuestion(exerciseId, editingQuestionId, payload);
      }
      return teacherApi.addExerciseQuestion(exerciseId, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teacher-exercise", exerciseId] });
  ...
  const deleteQuestionMutation = useMutation({
    mutationFn: (questionId: number) =>
      teacherApi.deleteExerciseQuestion(exerciseId, questionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teacher-exercise", exerciseId] });
        ...
        {exercise && (
          <>
            ...
            <div className="flex flex-col gap-2">
              <Link href={`/teacher/exercises/${exerciseId}/submissions`}>
                <Button
                  variant="outline"
