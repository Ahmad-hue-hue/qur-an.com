...
export default function TeacherExercisesPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (exercise) => {
      ...
      router.push(`/teacher/exercises/${exercise.id}`);
    },
    onError: (err: Error) => toast.error(err.message || "Create failed"),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => teacherApi.deleteExercise(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teacher-exercises"] });
        ...
        {exercises?.map((exercise) => (
          <Card key={exercise.id} className="card-shadow">
            <CardContent className="p-4 space-y-3">
              ...
              <Link href={`/teacher/exercises/${exercise.id}/submissions`}>
                <Button
                  variant="outline"
